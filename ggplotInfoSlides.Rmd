...
```{r bars2, message=FALSE, results='markup', fig.height=5}        
bars <- ggplot(liefer) + 
           geom_bar(mapping=aes(x=Lieferzeiten),
                    fill=myPalette[3])
...
```{r bars5a, message=FALSE, echo=FALSE, results='markup', fig.height=6 , warning=FALSE }
ggplot(liefer) +
  geom_bar(mapping=aes(x=Lieferzeiten, y = ..prop..), 
           fill=myPalette[3]) +
  scale_x_discrete(limits=c(1,2,3,4,5,6,7)) +  
...
# Bessere Wahl der Klassenbreite
