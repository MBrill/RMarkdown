---
title: "Hypothesentests mit zwei Stichprobe"
...
wir eine Abbildung betrachtet, in der wir zwei Stichproben verglichen hatten und die Frage gestellt hatten, ob die beiden Mittelwerte
der Stichproben sich signifikant unterscheiden. Diese Frage können wir 
...
Als Beispiel betrachten wir das Beispiel einer Supermarktkette, die eine neue Filiale öffnen möchte. Zur Diskussion stehen zwei Standorte.
Deshalb werden in der Nähe beider Standorte Stichproben über die Einkommen der dortigen Bewohner gezogen. Dabei gehen wir davon aus,
...
Als Nullhypothese verwendet man hier meist
\[
\mu_1 - \mu_2 = D_0.
\]
...
\[
\sigma_D = \sqrt{\frac{\sigma_1^2}{n_1} + \sigma_2^2}{n_2}}.
\]
...
dass die Varianz in beiden Stichproben gleich oder verschieden ist.
Für beide Fälle gibt es dann schätzer für die Varianz der Differenz,
...
Stichproben zweier normalverteilter Zufallsvariabler. Als Irrtumswahrscheinlichkeit verwenden wir hier 1%.
...
In der Ausgabe der Funktion *t.test* finden wir Angaben über die berechneten Mittelwerte der beiden Stichproben und ein Prognose-Intervall für die Differenz. Im Beispiel erhalten wir das
Intervall [`r result$conf.int`]. Mit Wahrscheinlichkeit 90%
liegt die Differenz der beiden Mittelwerte in diesem Intervall. Daraus schließen wir mit der Wahrscheinlichkeit `r result$p.value`, dass der Mittelwert
...
Nehmen wir die Nullhypthose, dass die Mittelwerte für die Wahrscheinlichkeit der Inhaftierung gleich ist, an?
Schon der ausgegebene p-Wert von `r result$p.value` sagt uns,
...
Wir erhalten nur eine Ausgabe bei der Schätzung der Differenz.
Diest ist mit `r result$estimate` auch so groß, dass wir die Nullhypothese der gleichen Arbeitslosenquoten verwerfen.
...
Tests nochmals auf der Basis von Daten, die [@mcclave_91] entnommen sind (Seite 421).
...
Der p-Wert bleibt unverändert und sehr hoch. Das Prognose-Intervall ist immer noch sehr groß. Wir würden also immer noch die Nullhypothese nicht verwerfen.

Wenn wir die Daten nochmals ausgeben, dann erkennen wir, dass die Umsätze in Restaurant 1 an jedem der Tage. Deshalb sollte es uns eigentlich schwer fallen den Ergebnissen der beiden Tests zu glauben.
...
Mit diesen Ergebnissen verwerfen wir unsere Nullhypothse. Der p-Wert ist extrem klein, und sowohl die Schätzung für die Differenz (`r result$estimate`) als auch das Prognose-Intervall für die Differenz
