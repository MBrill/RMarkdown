...
nehmend die Gegenhypothese an, dass die Füllhöhe weniger als 500 beträgt.
...
arithmeischen Mittel über diese Zählvariable eine Punktschätzung
für p. Ist n groß genug, dann können wir die Binomialverteilung
durch eine Normalverteilung ersetzen.

Bevor wir dies durchführen rufen wir uns in Erinnerung:
\[
E(X) = n \cdot p, \sigma^2 = \frac{p(1-p)}{n}.
\]
Mit dem arithmetischen Mittel berechnen wir eine Schätzung
p, , die Standardabweichung können wir mit der in der Nullhypothese
angegebenen Anteil berechnen und führen anschließend eine Gauß-Test
für diese Statistik durch:
\[
H_0: p = p_0, H_1: p \neq p_0.
\]
Wir berechnen die folgende standardnormalverteilte Statistik:
\[
...
Mit den Ausgaben der Funktion erkennen wir, dass wir die Nullhypthese
nicht verwerfen.
...
Auch hier verwerfen wir die Nullhypothese nicht. Wir können wie schon
bei den Gauß- und t-Tests die einzelnen Ausgaben in der Variable 
*res* abfragen. Als Wert für die Statistik erhalten wir
`r res$estimate`, und als Annahmebereich das Intervall
[`r res$conf.int`]. Auch der berechnete p-Wert von `r res$p.value`
spricht dafür, die Nullhypothese nicht zu verwerfen.
