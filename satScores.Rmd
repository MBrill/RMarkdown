...
zwei Stichproben oder eine Spalte, die wir mit einem Faktor in zwei
Teile aufteilen können darauf überprüfen können, ob die Erwartungswerte
...
die Variationsanalyse (ANOVA) ein. Wir erhalten bei unseren
...
eines Hypothesentests untersucht, der ANOVA verwendet.
...
der Daten gleich sind und sich aber je nach Geschlecht unterscheiden.
...
die Nullhypothese in den anschließenden Hypothesentests verworfen werden wird oder nicht.
Dazu verwenden wir einmal *ggplot* und stellen die Werte als Punkte auf dem Zahlenstrahl aus.
...
Wir gehen davon aus, dass die Varianzen in beiden Teilen übereinstimmen. Das Ergebnis, das wir erhalten wird nicht wesentlich beeinflusst, wovon Sie sich überzeugen können. Aber wir hatten bei der
Untersuchung der Streuung festgestellt, dass die Standardabweichungen in beiden Teilen
der Teilnehmer übereingestimmt hatten.
...
Wir betrachten jetzt den Fall 2. Hier würden wir vermuten, dass die Nullhypothese verworfen wird:
...
Die Ausgaben von R bestätigen diese Vermutung, wir verwerfen die Nullhypothese und gehen jetzt davon aus,
...
Dazu berechnen wir zuerst die Streuung der Mitelwerte der beiden Teilmengen für
...
Wir formulieren dies direkt allgemein für mehr als 2 Teile, als Symbol verwenden
...
Für unser Beispiel erhalten wir einen Wert SST = `r sst`.
...
F = \frac{MST}{MSE}
\]
Ist der Quotient F nahe beim Wert 1 weisen darauf hin, dass die Streuung innerhalb der
Gruppen und die Streuung im kompletten Datensatz sehr nahe beieinander liegen.
Dann könnten die Unterschiede zwischen den verschiedenen Mittelwerte in den Gruppen
einfach durch zufällige Einflüsse hervorgerufen werden. Anders sieht das aus wenn
...
erhalten wir einen Wert von `r fvalue` für den Quotienten.
...
Wir berechnen die Quantile der F-Verteilung. Dazu haben wir zwei Freiheitsgrade
für den Wert von p-1 und für n-p ein Zähler und Nenner. Dies sind die beiden Zahlen
die wir bereits in der Berechnung von Zähler und Nenner verwendet haben.
Wir berechnen das 95% Quantil:


```{r  fquant, include=TRUE, echo=FALSE}
fquant <- qf(0.95, 1, 8)
```
...
Der p-Wert führt dazu, dass wir wieder die Nullhypothese nicht verwerfen. Im Eintrag
*F value* finden wir unseren Zahlenwert wieder.
...
Wieder wird die Nullhypothese verworfen, dafür spricht auchb der angegebene p-Wert.
