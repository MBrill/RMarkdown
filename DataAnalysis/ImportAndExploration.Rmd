---
title: "Erste Schritte bei der Datenanalyse"
...
## Datentabellen
Als erstes Widget verwenden wir die Library *DT* und die darin
...
ggplot(Auto) + 
  geom_bar(mapping=aes(x=Region, y = after_stat(prop), group=TRUE),
           fill=myPalette[1]) +
  labs(
...
Verbrauchsangaben in Abhängigkeit vom Baujahr. Das ist eine sogenannte
Zeitreihe - auf der x-Achse sind Datumsangaben, auf der y-Achse die Werte.
Typische Zeitreihen sind zum Beispiel die Börsenkurse.

Wir haben für jedes Baujahr eine Menge von Einträgen. Damit wir
einen zeitlichen Verlauf finden berechnen wir für jedes Baujahr 
den Median und stellen den Verlauf der Mediane dar.

```{r HubraumMediane, message=FALSE, echo=TRUE}
Jahreswerte <- Auto %>%
  group_by(Baujahr) %>%
  summarize(HubMedian = median(Hubraum), VerbrauchMedian = median(Verbrauch))


Jahreswerte
```
Schon die Ausgabe der Werte zeigt uns,
dass wir hier keinen sinkenden oder steigenden Trend erwarten können.

Wir erstellen im ersten Versuch einen Polygonzug mit ggplot2.

```{r TimelineGGHubraum, message=FALSE, echo=TRUE}
ggplot(Jahreswerte, aes(x = Baujahr, 
                        y = HubMedian)) + 
  geom_line() +
  geom_point() + 
  labs(
    title="Baujahr und Hubraum",
    x = "Baujahr",
    y = "Hubraum in Liter"
  )   
```

```{r TimelineVerbrauch, message=FALSE, echo=TRUE}
ggplot(Jahreswerte, aes(x = Baujahr, 
                        y = VerbrauchMedian)) + 
  geom_line() +
  geom_point() + 
  labs(
    title="Baujahr und Verbrauch",
    x = "Baujahr",
    y = "Verbrauch in l/100 km"
  )   
```

Wir erzeugen die Werte nochmals, dieses Mal gruppieren wir aber nicht nur
nach dem Baujahr, sondern beziehen auch die Retion ein. Wir filtern
wie schon häufig und beschränken uns auf europäische Autos.

```{r HubraumMedianGruppierte, message=FALSE, echo=TRUE}
Jahreswerte <- Auto %>%
  group_by(Region, Baujahr) %>%
  summarize(HubMedian = median(Hubraum), VerbrauchMedian = median(Verbrauch))
...
Wir erstellen wieder eine Zeitreihe:

```{r TimelineVerbraucheuropa, message=FALSE, echo=TRUE}
Jahreswerte %>%
...
In Europa gibt es zwar keine durchgehende Trendlinie, aber
wie auch erwartet ist gut zu erkennen, dass über die Jahre
die Verbrauchsangaben sinken. Der angezeigte Verlauf
hat sicher auch mit den enthaltenen Werten zu tun.
...
## Bemerkung
Wir haben Zeitreihen dargestellt, dabei haben wir *geom_line* aus *ggplot2*
eingesetzt. Es gibt das HTMLWidget dygraph, mit dem solche Zeitreihen
interaktiv visualisierbar sind. Darauf haben wir hier verzichtet,
da die vorliegenden Daten nicht gut für dieses Widget geeignet sind.
Bei großen Zeitreihen ist die Verwendung von *dygraph* unbedingt empfohlen!
