...
Interessant ist das Package *rstatix*, zu dem es eine gute Web-Site und ein Buch gibt
...
Unsere Vorhersage ist bestätigt, mit diesen p-Werten verwerfen wir
die Nullhypothese, die Erwartungswerte in der Cholesterol-Reduktion
unterscheiden sich.

## Paarweise Vergleiche

Jetzt stellt sich sofort die Frage, welche Stufen bzw. Behandlungen sich von welchen signifikant unterscheiden.
...
```{r kabacoff2Save, include=FALSE, echo=FALSE}
...
```{r kabacoff2glhtSave, include=FALSE, echo=FALSE}
...
```{r TukeyAnovaPlotSave, include=FALSE, echo=FALSE}
...
In der Annahme für die Verwendung der Funktion *aov* steckt insbesondere,
...
```{r kabacoffqqSave, include=FALSE, echo=FALSE}
...
Die Nullhypothese für diese Hypothesentests ist, dass sich die 
...
Wir geben die ersten  Zeilen der Daten als Tabelle aus: 
...
Wir erkennen auch, dass es durchaus Unterschiede zwischen den verschiedenen Mittelwerten
der Zahnlängen gibt. Ob diese Unterschiede signifikant sind oder nicht muss der Hypothesentest
erweisen.

Bevor wir den Hypothesentest durchführen erstellen wir einen Boxplot,
...
```{r toothG1BoxSave, include=FALSE, echo=FALSE}
ggsave(filename="toothBox.png", plot=last_plot(), device="png",
       path="images/", width=16, height=9, units="cm")
```
Man erkennt auch hier durchaus eine Unterscheidung, insbesondere scheint es einen Zusammenhang zwischen der verwendeten Dosis und der Länge der Zähne
zu geben.
...
```{r toothQQSave, include=FALSE, echo=FALSE}
...
```{r toothQQ2Save, include=FALSE, echo=FALSE, warnings=FALSE}
ggsave(filename="toothqqgroups.png", plot=last_plot(), device="png",
       path="images/", width=16, height=9, units="cm")
```

## Hypothesentest
Jetzt führen wir die Varianzanalyse durch. Dabei verwenden wir eine Pipeline
und die Funktion *anova_test*.
...
Auch diese Interaktion ist signifikant. Wir verwerfen die Nullhypothese.
...
Wir erstellen 
einen Boxplot, indem wir dies gut erkennen:
...
```{r BonferroniAnovaPlotSave, include=FALSE, echo=FALSE}
...
kurz betrachten werden. Eine gute Beschreibung dafür finden wir auf [@kassambara].
...
die nochmals unterteilt werden, darauf hin ob ihr Migräne-Risiko klein oder groß ist.
...
```{r threeBoxSave, include=FALSE, echo=FALSE}
...
Es gibt eine signifikante Interaktionen zwischen allen drei Faktoren. Und es gibt
zwei nicht-signifikante Interaktionen, einmal zwischen Geschlecht und Migräne-Risiko
und zwischen Migräne-Risko und Medikament.

Anschließend führen wir eine Gruppierung nach Geschlecht durch und innerhalb der so entstehenden 
...
Anschließend kann man für die männlichen Teilnehmer weiter analysieren und paarweise Vergleiche anstellen und visualieren, worauf wir an dieser Stelle jedoch verzichten.
...
für einen solchen ANCOVA-Test ist, dass es einen linearen Zusammenhang
zwischen *gesttime* und *weight* gibt. Wenn man in diesem Beispiel die Ausgleichsgeraden bestimmt
