---
title: "Der Datensatz Tips - Vorhersagen mit linearen Modellen"
...
# Ausgabe von double-Werte mit  drei Stellen hinter dem Komma
options(digits=3)
```

# Der Datensatz Tips
Der Datensatz enthält Angaben über Trinkgelder in einem US-amerikanischen Familienrestaurant aus den neunziger Jahren. Ursprünglich stammen die Daten aus [@bryant_95]. Der Datensatz wird in vielen Büchern zur Statistik und Datenanalyse zitiert. Die vorliegende Darstellung folgt [@cook_07] und [@theus_09],
...
ggplot(daten, 
  ...
  geom_jitter(color=myPalette[6],
             ...
             width=0.1,
             height=0.0) +
  scale_x_discrete(breaks = c("Donnerstag", "Freitag", "Samstag", "Sonntag")) +  
  labs(
...
die durch das lineare Moell gegebene Ausgleichsgerade direkt mit *ggplot* ausgeben können.
...
die Ergebnisse in der Grafik mit Funktionen wie *geom_abline* dar.
...
ggsave(filename="tipsResid.png", 
       ...
       units="cm")
```
Die Werte die wir einsetzen möchten und für die wir
eine Vorhersage erwarten speichern wir in einem tibble:
```{r predictionx, echo=TRUE}
persons <- tibble(Anzahl = c(4,6,7,8))
```

Wir erzeugen Vorhersagen mit Hilfe der Funktion *predict*. 
...
Wir verwenden das berechnete lineare Modell und das Tibble
mit den Personenanzahlen.
...
Wir machen jetzt Vorhersagen für verschiedene Anzahlen von Personen an einem Tisch
und einer gegebenen Rechnungshöhe.
...
# Supervised Machine Learning für Vorhersagen
...
Die Empfehlung für das bisher aufgestellte Modell stammt
aus Lehrbüchern, in denen Regressionsrechnung für
die Stochastik betrachtet wurde. 
Jetzt werden wir diesen Ansatz erweitern und
Machine Learning betrachten.
Hier können wir nachwievor Modelle mit einer Einflussvariable
einsetzen, aber die in R verfügbaren Funktionen erlauben
es uns, eine ganze Menge von Einflussvariablen in das Modell
aufzunehmen.
...
Wir machen einige Vorhersagen, wie schon für unser erstes Modell. 
Die Anzahl der Personen, für die wir die Vorhersagen machen möchten
speichern wir in einem Tibble ab. 
Wir betrachten Vorhersagen für `r persons$Anzahl[1]`, `r persons$Anzahl[2]`, `r persons$Anzahl[3]` und `r persons$Anzahl[4]` Personen.
...
Der Residualplot für dieses Modell sieht, bis auf einige Ausreißer für sehr kleine und sehr große Rechnungshöhen, ebenfalls gut aus, so dass wir jetzt ein zweites lineares Modell für die Vorhersage des
Trinkgelds besitzen. Wir vergleichen nochmals beide Vorhersagen
in einer Tabelle.
...
Bevor wir einen Blick darauf werfen, wie wir verschiedene Modelle vergleichen um
eine Entscheidung treffen, welches davon wir letztendlich einsetzen, erstellen wir
...
sowohl die Anzahl der Personen und die Rechnungshöhe in ein Modell als Einflussgrößen einzubauen. Ein solches Modell
sollte dann auch Wechselwirkungen zwischen beiden Einflussgrößen
modellieren können.
...
# Auswahl der Einflussgrößen für ein Modell
